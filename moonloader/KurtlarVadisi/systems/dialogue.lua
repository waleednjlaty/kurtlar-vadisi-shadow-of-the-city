...
function M.load()
 for _,id in ipairs({'title','memati_001','polat_001','envelope','wolves','objective','complete','selim_photo'}) do
  local p=getGameDirectory()..'\\moonloader\\KurtlarVadisi\\ui\\'..id..'.png'
  if not M.textures[id] then
...
function M.draw()
 ...
 if id=='envelope' then
  local ph=h*.66;local pw=ph*.75;local x=(w-pw)/2;local y=h*.12
  renderDrawBox(x-12,y-12,pw+24,ph+85,0xFFF1EBDD)
  renderDrawTexture(M.textures.selim_photo,x,y,pw,ph,0,0xFFFFFFFF)
  renderDrawTexture(M.textures.envelope,x,y+ph,pw,70,0,0xFFFFFFFF)
...
function M.dispose() for _,h in pairs(M.textures) do pcall(renderReleaseTexture,h) end;M.textures={};M.clear() end
return M
