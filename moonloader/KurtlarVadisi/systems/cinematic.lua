...
function M.begin() audio.stopFreeRoam();M.active=true;setPlayerControl(PLAYER_HANDLE,false);M.black=1 end
...
function M.restore()
 M.active=false;M.black=0
 pcall(restoreCameraJumpcut)
 if doesCharExist(PLAYER_PED) then pcall(setPlayerControl,PLAYER_HANDLE,true) end
end
function M.draw()
